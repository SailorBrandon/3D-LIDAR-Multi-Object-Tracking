...
double CAR_LENGTH = 0.35;    // 车的宽高
double CAR_WIDTH = 0.2;
// costmap paramter

// 初始化网格Grid状态
void mapCartesianGrid(PointCloud<PointXYZ>::Ptr elevatedCloud,
                      array<array<int, numGrid>, numGrid> &cartesianData) {
  array<array<int, numGrid>, numGrid>
      gridNum{};  //  gridNum 此数组专门统计落在每个grid的点云数，
                  //  gridNums[250][250]是指：
                  //  elevatedCloud点的XY平面离散为m×n单元的网格（见paper图
                  //  Figure4-5）
  ...
  for (int i = 0; i < elevatedCloud->size(); i++) {  // 遍历高点数
    ...
    // exclude outside roi points  排除外部roi points  x,y属于(-25,
    // 25)下面才继续执行
    if (xC < 0 || xC >= roiM || yC < 0 || yC >= roiM)
      continue;  // continue后，下面不执行。gridNum[xI][yI] 值不变
                 // 限制范围（0，50）
    int xI =
        floor(numGrid * xC / roiM);  //  xI .yI    const int numGrid = 250;
                                     //  floor(x)返回的是小于或等于x的最大整数
    int yI = floor(numGrid * yC / roiM);  // 50x50 映射到→250x250
    gridNum[xI][yI] =
        gridNum[xI][yI] + 1;  // 统计落在这个grid的有多少个点！！！
    //   cout << "gridNum[xI][yI]: " << gridNum[xI][yI] <<
    //   "-------------统计落在这个grid的有多少个点！！！------------" << endl;
    //   // gridNum[xI][yI]
  ...
  // 然后所有相邻的相邻像元（即x-1，y  + 1，x，y +1，x +1，y +1 x -1，y，x
  // +1，y，x -1，y -1，x，检查y − 1，x + 1，y +
  // 1）的占用状态，并用当前集群ID标记。
  ...
  for (int xI = 0; xI < numGrid; xI++) {  //   const int numGrid = 250;
    for (int yI = 0; yI < numGrid; yI++) {
      if (gridNum[xI][yI] > 1) {  // 一个点的直接舍弃？
        cartesianData[xI][yI] =
            -1;  // 网格分配有2种初始状态，分别为空（0），已占用（-1）和已分配。随后，将x，y位置的单个单元格选作中心单元格，并且clusterID计数器加1
        //下面为设置当前点的周围点数值为-1
        if (xI == 0) {
          if (yI == 0) {
  ...
  // for(int xI = 0; xI < numGrid; xI++){  //   const int numGrid = 250;
  //     for(int yI = 0; yI < numGrid; yI++){
  //          cout << " cartesianData[xI][yI] is "<< xI<<"   " << yI  <<"   " <<
  //          cartesianData[xI][yI]<< "========设置为-1========" <<endl;
  //     }}
}

// findComponent会引用search函数   聚类    图搜索  这是什么聚类方法？？？
void search(array<array<int, numGrid>, numGrid> &cartesianData, int clusterId,
            int cellX, int cellY) {  //  cellX(0-249), cellY(0-249)
  cartesianData[cellX][cellY] =
      clusterId;  // 赋值，将周边邻居值赋值同样的clusterId
...
void findComponent(array<array<int, numGrid>, numGrid> &cartesianData,
                   int &clusterId) {
  for (int cellX = 0; cellX < numGrid;
       cellX++) {  // 循环每个点   numGrid = 250;
    for (int cellY = 0; cellY < numGrid; cellY++) {
      //   cout << "cartesianData[cellX][cellY]  is "<< cellX<<"   " << cellY
      //   <<"   "  <<  cartesianData[cellX][cellY]  <<endl;   //
      //   大多数是0??为什么
      if (cartesianData[cellX][cellY] ==
          -1) {  // 随后，将x，y位置的单个单元格选作中心单元格，并且clusterID计数器加1
                 // (网格分配有2种初始状态，分别为空（0），已占用（-1）)
        clusterId++;  // 对m×n网格中的每个x，y重复此过程，直到为所有非空cluster分配了ID。
                      // // cout << "clusterId is "<< clusterId <<endl;  //
                      // 特别少 初始聚类数量
        search(cartesianData, clusterId, cellX,
               cellY);  // 对每一个点进行搜索  cellX(0-249), cellY(0-249)
      }
    }
  }
}
...
void componentClustering(PointCloud<pcl::PointXYZ>::Ptr elevatedCloud,
  ...
  // 在这种情况下，30mx30m带有100x100x网格
  mapCartesianGrid(
      elevatedCloud,
      cartesianData);  // 第一步设置网格Grid状态   网格数组：cartesianData
...
void makeClusteredCloud(PointCloud<pcl::PointXYZ>::Ptr &elevatedCloud,
  ...
  for (int i = 0; i < elevatedCloud->size(); i++) {
    ...
    // cout << "xI is "<< xI <<endl;
    // cout << "yI is "<< yI <<endl;
    // cout << "cartesianData is "<< cartesianData[xI][yI]<<endl;  //
    // (1,2,3,4,...,numCluster)
    int clusterNum =
        cartesianData[xI][yI];  //  数值  每一点云点对应的栅格聚类数字标签
    if (clusterNum != 0) {
      PointXYZ o;
      o.x =
          grid_size * xI - roiM / 2 +
          grid_size / 2;  // 网格大小？？ roiM = 50  grid_size = (0.200000003F)
      o.y =
          grid_size * yI - roiM / 2 + grid_size / 2;  // 转换成（-25  ~ 25）范围
      o.z = -0.1;                                       // 高度统一设置为-1
      // o.r = (500*clusterNum)%255;   // 不同类不同颜色   error: ‘struct
      // pcl::PointXYZ’ has no member named ‘r’，用pcl::PointXYZRGBA？ o.g =
      // (100*clusterNum)%255; o.b = (150*clusterNum)%255;
      clusterCloud->push_back(o);  //
    }
  }
}
...
std::vector<int> createCostMap(const pcl::PointCloud<pcl::PointXYZ> &scan) {
  ...
  for (const auto &p : scan.points) {
    if (p.z > HEIGHT_LIMIT) continue;
    if (std::fabs(p.x) < CAR_LENGTH && std::fabs(p.y) < CAR_WIDTH) continue;

    // Calculate grid index
    int grid_y = (p.x + map_center_x) / g_resolution;
    int grid_x = (p.y + map_center_y) / g_resolution;
    if (grid_y < 0 || grid_y >= g_cell_width || grid_x < 0 ||
        grid_x >= g_cell_height)
      continue;
    ...
    if (cost_map[index] > 100) cost_map[index] = 100;
  }
