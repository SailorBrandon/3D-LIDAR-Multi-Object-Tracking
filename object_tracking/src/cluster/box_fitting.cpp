...
float sensorHeight = 0.35;  // 激光雷达距离地面高度
...
void getClusteredPoints(PointCloud<PointXYZ>::Ptr elevatedCloud,
  ...
  for (int i = 0; i < elevatedCloud->size(); i++) {
    ...
    int clusterNum =
        cartesianData[xI][yI];  // 1 ~ numCluster   初始聚类ID数量numCluster
    ...
    if (clusterNum != 0) {
      ...
      o.y = y;
      o.z = z;
      clusteredPoints[vectorInd].push_back(
          o);  // 填充clusteredPoints   PointXYZ类型数组
...
//
void getPointsInPcFrame(Point2f rectPoints[], vector<Point2f>& pcPoints,
...
bool ruleBasedFilter(vector<Point2f> pcPoints, float maxZ, int numPoints) {
  ...
  if (numPoints < 30)
    return isPromising;  // 小于30个点，返回false 舍弃此聚类  cout << "numPoints
                         // is "<< numPoints <<endl;  // 差不多(3~, 800~)
  ...
  if (height > tHeightMin &&
      height < tHeightMax) {  //(0.8  2.6) tHeightMin = 0.8;  tHeightMax = 2.6;
...
// getBoundingBox引用
// 制作实体框====================输入为clusteredPoints[iCluster]：每个聚类的所有三维点坐标
visualization_msgs::Marker mark_cluster(
  ...
  pcl::compute3DCentroid(cloud_cluster,
                         centroid);  // 坐标 centroid- 重心   / /计算点云中心
  pcl::getMinMax3D(cloud_cluster, min,
                   max);  // 得到它x,y,z三个轴上的最大值和最小值
...
void getBoundingBox(vector<PointCloud<PointXYZ>> clusteredPoints,
  ...
  // 初始聚类ID数量numCluster is
  std::cout << "***boxFitting***" << std::endl;
  for (int iCluster = 0; iCluster < clusteredPoints.size();
       iCluster++) {  //  循环的次数就是 初始聚类ID数量numCluster
    Mat m(picScale * roiM, picScale * roiM, CV_8UC1, Scalar(0));
    float initPX = clusteredPoints[iCluster][0].x +
                   roiM / 2;  // (0 ~ 50) 第0个点的x，y坐标
    ...
    int numPoints =
        clusteredPoints[iCluster].size();  //  每一聚类里面有多少点 numPoints
    ...
    // 将最小面积矩形（MAR）[128]应用于每个聚类对象，从而生成一个2D框，
    // 某些稀疏点（带红色圆圈）被认为是异常值，并且MAR过程导致不正确的框，使用L形拟合可解决此问题
    // 可参考:https://www.yuque.com/huangzhongqing/hre6tf/pcohs1#FONbX
    float xDist = maxMx - minMx;  // 选择两个最远的离群点x1和x2
    float yDist = maxMy - minMy;
    float slopeDist = sqrt(
        xDist * xDist +
        yDist *
            yDist);  // 选择两个最远的离群点x1和x2，它们位于面向LIDAR传感器的对象的相对侧。然后在两点之间绘制一条线Ld
    /////////////////////std::cout << "boxFitting  slopeDist" << slopeDist <<
    /// std::endl;
    ...
    // start l shape fitting for car like object 开始为汽车之类的物体 l shape
    // fitting lSlopeDist = 10000, lnumPoints = 30000;
    if (slopeDist > lSlopeDist && numPoints > lnumPoints) {  // int lSlopeDist = 1.0;  int lnumPoints =
                                      // 5;判断框的大小以及框内的点数
      // if(1){
      float maxDist = 0;
      float maxDx, maxDy;
      ...
      // get max distance
      for (int pInd = 0; pInd < clusteredPoints[iCluster].size(); pInd++) {
        ...
        float dist = abs(slope * xI - 1 * yI + maxMy - slope * maxMx) /
                     sqrt(slope * slope + 1);  //点到直线的距离
      ...
      //     /点到直线的距离方程 float dist = abs(slope * xI - 1 * yI + maxMy -
      //     slope * maxMx) /
      //                  sqrt(slope * slope + 1);  //点到直线的距离
      //     if (dist > maxDist) {                  // 寻找最远的距离  maxDist
      //       maxDist = dist;  // 赋值给最远的距离  maxDist
      ...
      // std::cout << " pcPoints[0]  " << pcPoints[0].x << " " <<  pcPoints[0].y
      // << std::endl; std::cout << " pcPoints[1]  " << pcPoints[1].x << " " <<
      // pcPoints[1].y << std::endl; std::cout << " pcPoints[2]  " <<
      // pcPoints[2].x << " " <<  pcPoints[2].y << std::endl; std::cout << "
      // pcPoints[3]  " << pcPoints[3].x << " " <<  pcPoints[3].y << std::endl;

      // std::cout << "boxFitting  maxZ " << maxZ << std::endl;   //  boxFitting
      // maxZ 0.525725
      //  最后一步:消除大多数无关对象，例如墙壁，灌木丛，建筑物和树木。实现尺寸阈值化以实现此目的。
      bool isPromising = ruleBasedFilter(
          pcPoints, maxZ,
          numPoints);  //   比如聚类33，每一类里面有多少点
                       //   numPoints(四个点,z最大值, 每个聚类中的点数)
      ...
      //     // for visualization
      //    for( int j = 0; j < 4; j++ )
      //        line( m, rectPoints[j], rectPoints[(j+1)%4ne],
      //        Scalar(255,255,0), 1, 8 );
      //    imshow("Display Image", m);
      //    waitKey(0);
      /////////std::cout << " else  pcPoints[0]  " << pcPoints[0].x << " " <<
      /// pcPoints[0].y << std::endl;
      ////////std::cout << " else  pcPoints[1]  " << pcPoints[1].x << " " <<
      /// pcPoints[1].y << std::endl;
      ///////std::cout << " else  pcPoints[2]  " << pcPoints[2].x << " " <<
      /// pcPoints[2].y << std::endl;
      ///////std::cout << " else  pcPoints[3]  " << pcPoints[3].x << " " <<
      /// pcPoints[3].y << std::endl;
    }
    ...
    visualization_msgs::Marker mac = mark_cluster(
        clusteredPoints[iCluster]);  // 边框参数设置(还在for循环里面哈)
                                     // 返回marker
    ma.markers.push_back(mac);  // ma在这里 填充  实体框  最新产生的实体框选

  }  // 循环截止     循环的次数就是 初始聚类ID数量numCluster
  // std::cout << "boxFitting   bbPoints:" << bbPoints.size() << " 多少个边界框"
  // << std::endl;
}

//  候选框拟合处理
vector<PointCloud<PointXYZ>> boxFitting(
    PointCloud<PointXYZ>::Ptr elevatedCloud,
    array<array<int, numGrid>, numGrid> cartesianData, int numCluster,
    visualization_msgs::MarkerArray& ma)  // ma数据
{
  vector<PointCloud<PointXYZ>> clusteredPoints(numCluster);
  getClusteredPoints(
      elevatedCloud, cartesianData,
      clusteredPoints);  // clusteredPoints：[聚类label数字][每个聚类的点坐标]
  vector<PointCloud<PointXYZ>> bbPoints;
