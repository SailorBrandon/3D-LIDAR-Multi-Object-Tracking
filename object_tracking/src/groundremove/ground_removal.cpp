...
void filterCloud(PointCloud<PointXYZ>::Ptr cloud,
  ...
  for (int i = 0; i < cloud->size(); i++) {
    ...
    if (distance <= rMin || distance >= rMax) {  // 判断欧氏距离，过滤去除异常点
                                                 // （ rMin = 3.4; rMax = 120;）
      continue;  // filter out
    } else {
      pcl::PointXYZ o;
...
void getCellIndexFromPoints(float x, float y, int& chI, int& binI) {
  ...
  // normalize(极坐标)
  float chP = (atan2(y, x) + M_PI) /
              (2 * M_PI);  // 范围（0，1）atan2(y, x)是4象限反正切 M_PI==3.14
  float binP =
      (distance - rMin) /
      (rMax - rMin);  //范围（0，1）  rMax - rMin ~~ (3.4, 120)  见figure4-4
  // index
...
void createAndMapPolarGrid(PointCloud<PointXYZ> cloud,
  ...
  for (int i = 0; i < cloud.size(); i++) {
    ...
    int chI, binI;
    getCellIndexFromPoints(x, y, chI,
                           binI);  //  xy坐标得到对应栅格坐标 得到CellIndex  ：
                                   //  chI, binI（极坐标：角度和半径）
    // TODO; modify abobe function so that below code would not need
    if (chI < 0 || chI >= numChannel || binI < 0 || binI >= numBin)
      continue;  // to prevent segentation fault 去除不合理点
    polarData[chI][binI].updateMinZ(
        z);  // {if (z < minZ) minZ = z;}  //
             // 更新得到z值点的最小值，最终得到最小值
             // ？？好几个点都在一个栅格，这些点只更新了updateMinZ
             // ,点的其他信息不存储？
  }
}
...
void computeHDiffAdjacentCell(array<Cell, numBin>& channelData) {
  ...
  for (int i = 0; i < channelData.size(); i++) {  // 120
    ...
    if (i == 0) {
      float hD =
          channelData[i].getHeight() - channelData[i + 1].getHeight();  // 差值
    ...
    // non-edge case
      ...
      float postHD =
          channelData[i].getHeight() - channelData[i + 1].getHeight();
      if (preHD > postHD)
    ...
    //    cout <<channelData[i].getHeight() <<" " <<channelData[i].getHDiff() <<
    //    endl; // 2, 0...
  }
}
    ...
    for (int bin = 1; bin < polarData[0].size() - 1; bin++) {
      if (!polarData[channel][bin].isThisGround()) {  // 判断不是地面
        ...
        if (polarData[channel][bin + 1].isThisGround() &&  // 四周是地面
            ...
            polarData[channel - 1][bin].isThisGround()) {
          vector<float> sur{polarData[channel][bin + 1]
                                .getHeight(),  // target cell is non-ground AND
                                               // surrounded by ground cells
                            polarData[channel][bin - 1].getHeight(),
                            polarData[channel + 1][bin].getHeight(),
                            polarData[channel - 1][bin].getHeight()};
          sort(sur.begin(), sur.end());  // 从小到大排序
          float m1 = sur[1];
          float m2 = sur[2];             // 取中间2个值
          float median = (m1 + m2) / 2;  // 取中值
          polarData[channel][bin].updataHeight(
              median);  //高度取中值  缺失单元的高度值将替换为相邻单元的中值
  ...
  for (int channel = 1; channel < polarData.size() - 1; channel++) {
    for (int bin = 1; bin < polarData[0].size() - 2; bin++) {
      if (polarData[channel][bin].isThisGround() &&  // 都是Ground
        ...
        if (height1 != tHmin && height2 == tHmin &&
            height3 != tHmin) {  // float tHmin = -2.0;  // 高度
          float newH = (height1 + height3) / 2;        //取两边均值
          polarData[channel][bin].updataHeight(newH);  // 更新高度值
          polarData[channel][bin]
              .updateGround();  //     void updateGround(){isGround = true;
                                //     hGround = height;}
        } else if (height1 != tHmin && height2 == tHmin && height3 == tHmin &&
                   height4 != tHmin) {
...
void groundRemove(PointCloud<pcl::PointXYZ>::Ptr cloud,          // 初始点云
  ...
  cout << "进入groundRemove函数中---------------------------------------"
       << endl;

  filterCloud(cloud,
              filteredCloud);  // 判断欧氏距离，过滤去除车周围半径外异常点
                               // （ rMin = 3.4; rMax = 120;）

  array<array<Cell, numBin>, numChannel>
      polarData;  // 数组定义polarData[numChannel][numBin]  ,Cell是个class
  ...
  for (int channel = 0; channel < polarData.size(); channel++) {  // channel: 80
    for (int bin = 0; bin < polarData[0].size(); bin++) {         //  120
      ...
      if (zi > tHmin && zi < tHmax) {
        polarData[channel][bin].updataHeight(zi);
      }  // 每个Cell栅格都有一个updataHeight
      else if (zi > tHmax) {
        polarData[channel][bin].updataHeight(hSeonsor);
      }  // float hSeonsor = 2;  高度     大于-0.4 ，设置为2
      else {
        polarData[channel][bin].updataHeight(tHmin);
      }  //  float tHmin = -2.0;  // 高度  小于-2，设置为-2
    }
    // could replace gauss with gradient
    //  computeGradientAdjacentCell(polarData[channel]);
    gaussSmoothen(polarData[channel], 1,
                  3);  // 高斯平滑  来自 src/groundremove/gaus_blur.cpp
    ...
    for (int bin = 0; bin < polarData[0].size(); bin++) {
      if (polarData[channel][bin].getSmoothed() <
              tHmax &&  // float tHmin = -2.0;
          polarData[channel][bin].getHDiff() <
              tHDiff) {  //  tHDiff = 0.4; 地面应该是相对平滑的
        polarData[channel][bin].updateGround();  // void updateGround(){isGround
                                                 // = true; hGround = height;}
  ...
  for (int i = 0; i < filteredCloud.size(); i++) {
    ...
    if (polarData[chI][binI].isThisGround()) {
      ...
      //   std::cout << " hGround "<< hGround << std::endl;  // 输出
      //   大多数都是-2 if (z > 0) std::cout << " z "<< z << std::endl;
      //   std::cout << " z "<< z << std::endl;  //
      if (z < (hGround +
               0.02)) {  // 判断hGround
                         // getHGround()）最终的判断重点就是getHeight 高度值
  ...
  cout << "初始点云 size: " << cloud->size()
       << " 处理后高点 size: " << elevatedCloud->size()
       << " 处理后地面点 size: " << groundCloud->size() << endl;
}